...
def mock_db():
    ...
    with (
        patch("coaching_mcp.tools.analyze_call.fetch_one") as mock_fetch_one,
...
class TestAnalyzeCallTool:
    ...
    def test_analyze_call_basic(self, mock_settings, mock_db):
        ...
        with patch(
            "coaching_mcp.tools.analyze_call.get_or_create_coaching_session"
        ) as mock_analysis:
    ...
    def test_analyze_call_with_transcript_snippets(self, mock_settings, mock_db):
        ...
        with patch(
            ...
            mock_analysis.return_value = {"snippets": [{"text": "Example snippet", "timestamp": 0}]}

            result = analyze_call_tool(call_id=call_id, include_transcript_snippets=True)
